...
import com.fido.config.MethodsConfig
...
//        val config: HookCustomMethodConfig,
        ...
        override fun onMethodEnter() {
            super.onMethodEnter()
            //在方法开始时插入自定义方法
//            val methodConfigs = config.filter { it.handleMode.hookMethodMode() == PluginConstant.ADD && classContext.currentClassData.className == it.originalClassName &&
//                    name == it.originalMethodName }
            val config = config.find { it.handleMode.hookMethodMode() == PluginConstant.ADD && classContext.currentClassData.className == it.originalClassName &&
                    name == it.originalMethodName
            }
            if (config != null) {
                ...
                LogPrint.normal(CUSTOM_METHOD_TAG){
                    """
                            ...
                            config.targetClassName =${config.targetClassName}
                            config.targetMethodName=${config.targetMethodName}
                        """.trimIndent()
                }
