...
val isNetworkAvailable: Boolean
  get() = app.getSystemService<ConnectivityManager>()?.run {
...
inline val isWifiEnabled: Boolean
  get() = app.getSystemService<WifiManager>()?.isWifiEnabled == true
...
class WifiListLiveData @RequiresPermission(allOf = [ACCESS_WIFI_STATE, CHANGE_WIFI_STATE]) constructor() : LiveData<List<WifiScanResult>?>() {
  ...
  override fun onActive() {
    app.registerReceiver(wifiScanReceiver, IntentFilter(WifiManager.SCAN_RESULTS_AVAILABLE_ACTION))
  }
